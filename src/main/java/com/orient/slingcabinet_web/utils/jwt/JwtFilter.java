...
import com.orient.slingcabinet_web.utils.common.JsonConvertUtil;
import com.orient.slingcabinet_web.utils.common.PropertiesUtil;
import com.orient.slingcabinet_web.utils.exception.CustomException;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        // 拿到当前Header中Authorization的AccessToken(Shiro中getAuthzHeader方法已经实现)
        String[] realToken = this.getAuthzHeader(request).split(" ");
        JwtToken token = new JwtToken(realToken[1]);
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        // 拿到当前Header中Authorization的AccessToken(Shiro中getAuthzHeader方法已经实现)
        String[] realToken = this.getAuthzHeader(request).split(" ");
        String token = realToken[1];
        // 获取当前Token的帐号信息
        String account = JwtUtil.getClaim(token, Constant.ACCOUNT);
        // 判断Redis中RefreshToken是否存在
        if (JedisUtil.exists(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            // Redis中RefreshToken还存在，获取RefreshToken的时间戳  记录的是即时登陆的时间戳
            String currentTimeMillisRedis = JedisUtil.getObject(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account).toString();
            // 获取当前AccessToken中的时间戳，与RefreshToken的时间戳对比，如果当前时间戳一致，代表是之前登录的用户且没有进行重新登录,
            // 因为重新登录的话 两个时间戳就不一样了
            // 进行AccessToken刷新
            if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                System.out.println("刷新后的token为==="+token);
